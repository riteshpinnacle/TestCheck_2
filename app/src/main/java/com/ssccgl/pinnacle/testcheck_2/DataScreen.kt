...
fun DataScreen(viewModel: MainViewModel = viewModel()) {
    ...
    // Store selected options and elapsed times for each question
    val selectedOptions = remember { mutableMapOf<Int, String>() }
    val elapsedTimeMap = remember { mutableMapOf<Int, Long>() }
    ...
    LaunchedEffect(Pair(isDataDisplayed, currentQuestionId)) {
        if (isDataDisplayed) {
            val currentQuestion = details.find { it.question_id == currentQuestionId }
            if (currentQuestion != null) {
                // Save selected option and elapsed time for the previous question
                selectedOptions[currentQuestionId] = selectedOption
                elapsedTimeMap[currentQuestionId] = elapsedTime

                // Initialize or resume the elapsed time for the new question
                elapsedTime = elapsedTimeMap[currentQuestionId] ?: 0L
                startTimeMap[currentQuestionId] = System.currentTimeMillis()
                        ...
                        tabTitles.forEachIndexed { index, title ->
                            Tab(
                                ...
                                onClick = {
                                    ...
                                    // Save the current state before switching tabs
                                    selectedOptions[currentQuestionId] = selectedOption
                                    elapsedTimeMap[currentQuestionId] = elapsedTime
                                    ...
                                    // Load the saved state for the new question
